...
## 摘要
运动预测是自动驾驶系统中的核心任务，旨在准确预测周围智能体的未来轨迹，以确保驾驶安全。现有方法通常独立处理离散的驾驶场景，忽略了现实驾驶环境中固有的时间连续性和历史上下文相关性。本文提出了 PanguMotion，这是一种针对连续驾驶场景的运动预测框架，它将盘古-1B（Pangu-1B）大语言模型中的 Transformer 模块作为特征增强模块集成到自动驾驶运动预测架构中。我们在通过 RealMotion 数据重组策略处理的 Argoverse 2 数据集上进行了实验，将每个独立的场景转换为连续序列，以模拟真实的驾驶场景。
