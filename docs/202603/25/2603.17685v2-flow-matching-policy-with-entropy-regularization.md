...
tldr: 针对扩散策略在强化学习中存在的熵控制难和计算开销大的问题，本文提出了基于流匹配的熵正则化策略（FMER）。该方法利用常微分方程（ODE）框架，通过最优传输理论构建直线概率路径进行动作采样，并推导出可计算的熵目标函数以实现原则性的最大熵优化。FMER在提升探索能力的同时，显著降低了训练时间，为复杂分布下的策略建模提供了高效方案。
selection_source: fresh_fetch
motivation: 旨在解决传统扩散策略在强化学习中难以精确控制熵以及迭代去噪导致计算开销巨大的局限性。
method: 采用流匹配参数化策略，利用优势加权的目标速度场引导更新，并结合可处理的熵正则化项增强探索。
result: "在FrankaKitchen稀疏奖励任务中表现优于SOTA，且训练速度比重型扩散基准快7倍，比高效变体快10-15%。"
conclusion: FMER通过流匹配实现了高效且具有原则性熵控制的在线强化学习，在复杂任务中兼顾了高性能与高计算效率。
---

## 摘要
基于扩散的策略因其表示复杂非高斯分布的能力，在强化学习（RL）中获得了极大的关注。基于随机微分方程（SDE）的扩散策略由于精确熵的不可计算性，往往依赖于间接的熵控制，同时在通过迭代去噪链进行策略梯度计算时，也面临着计算成本极高的问题。为了克服这些挑战，我们提出了带有熵正则化的流匹配策略（FMER），这是一种基于常微分方程（ODE）的在线强化学习框架。受最优传输理论的启发，FMER 通过流匹配对策略进行参数化，并沿着直线概率路径采样动作。FMER 利用模型的生成特性，从候选集中构建优势加权的目标速度场，从而引导策略更新向高价值区域迈进。通过推导可计算的熵目标，FMER 实现了有原则的最大熵优化，从而增强了探索能力。在稀疏多目标 FrankaKitchen 基准测试上的实验表明，FMER 的性能优于现有最先进的方法，同时在标准 MuJoco 基准测试中也极具竞争力。此外，与重型扩散基准（QVPO）相比，FMER 的训练时间缩短了 7 倍，与高效变体相比缩短了 10-15%。
