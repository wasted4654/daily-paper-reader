...
tldr: 针对扩散策略在强化学习中存在的熵控制难和计算开销大的问题，本文提出了基于流匹配的熵正则化策略（FMER）。FMER利用常微分方程（ODE）框架，通过最优传输理论构建直线概率路径，并推导出可计算的熵目标函数以增强探索。实验表明，FMER在复杂任务中表现优异，且训练速度显著快于传统的扩散策略基准，为生成式策略提供了更高效的优化路径。
selection_source: fresh_fetch
motivation: 旨在解决扩散策略在强化学习中由于熵不可计算导致的控制难以及迭代去噪带来的高昂计算成本问题。
method: 提出一种基于流匹配的ODE在线强化学习框架，利用优势加权的目标速度场引导更新，并推导出可处理的熵正则化项以增强探索。
result: "在FrankaKitchen多目标任务中优于SOTA方法，且训练速度比重型扩散基准快7倍，比高效变体快10-15%。"
conclusion: FMER通过流匹配实现了高效且具备原则性熵优化的策略学习，证明了直线概率路径在强化学习中的优越性。
---

## 摘要
基于扩散的策略因其能够表示复杂的非高斯分布，在强化学习（RL）中获得了极大的关注。基于随机微分方程（SDE）的扩散策略由于精确熵的难解性，通常依赖于间接的熵控制，同时在通过迭代去噪链进行策略梯度计算时，也面临着计算开销过大的问题。为了克服这些挑战，我们提出了带有熵正则化的流匹配策略（FMER），这是一种基于常微分方程（ODE）的在线强化学习框架。受最优传输理论的启发，FMER 通过流匹配对策略进行参数化，并沿着直线概率路径采样动作。FMER 利用模型的生成特性，从候选集中构建优势加权的目标速度场，从而引导策略更新向高价值区域靠拢。通过推导出一个可计算的熵目标，FMER 实现了基于原则的最大熵优化，以增强探索能力。在稀疏多目标 FrankaKitchen 基准测试上的实验表明，FMER 的性能优于现有最先进的方法，同时在标准 MuJoco 基准测试中也保持了竞争力。此外，与重型扩散基准（QVPO）相比，FMER 将训练时间缩短了 7 倍，相对于高效变体也缩短了 10-15%。
