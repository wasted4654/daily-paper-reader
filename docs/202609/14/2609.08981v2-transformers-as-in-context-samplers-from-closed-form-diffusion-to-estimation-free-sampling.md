...
tags: ["query:ardm"]
score: 6.0
evidence: Transformer在上下文学习中模拟迭代生成扩散采样器
...
motivation: 旨在探究Transformer是否不仅能进行判别式学习，还能在无需参数更新的情况下实现上下文数据生成。
method: 通过理论构造证明Transformer可实现闭式扩散采样，并结合语义主题采样任务分析隐藏层的几何结构与能量演化。
result: 实验发现Transformer隐藏层在处理语义主题时呈现出先趋向均匀球形分布、后回归主题结构的U型能量演化模式。
conclusion: Transformer具备实现复杂生成采样算法的内在能力，其层级结构与能量基采样器的动力学过程高度一致。
---
