...
tldr: 针对现有神经求解器在处理异构分布、长决策链及相似节点表示时鲁棒性不足的问题，本文提出一种基于约束矩阵扩散的生成式神经求解框架。该方法利用离散噪声图扩散模型学习车辆路径问题（VRP）的潜在约束并生成约束分配矩阵，随后将其作为图结构掩码自适应地集成到自回归求解器的特征学习与决策过程中。作为首个在CVRPlib大规模组合空间进行全面实验的研究，该模型在多个基准数据集上实现了最先进的性能，显著增强了求解器的全局视野与局部特征融合能力。
selection_source: fresh_fetch
motivation: 现有的基于注意力机制的神经求解器在面对异构分布、长决策周期及高相似度节点时，其鲁棒性和泛化能力存在明显局限。
method: 提出一种融合框架，通过离散噪声图扩散模型生成约束分配矩阵，并将其作为掩码嵌入自回归求解器以辅助决策。
result: 在CVRPlib数据集的378个组合空间维度上进行了全面测试，实验结果表明该模型在多个基准任务中均达到了当前最优水平。
conclusion: 结合扩散模型捕捉的约束信息与自回归求解过程，能有效提升神经求解器在复杂约束下的路径规划能力和鲁棒性。
---

## 摘要
过去十年中，由生成式人工智能驱动的神经网络求解器因其卓越的计算效率和出色的推理能力，在车辆路径问题（VRPs）领域引起了广泛关注。特别是，结合了强化学习的自回归求解器已成为一种显著趋势。然而，现有的大量工作强调神经方法的大规模泛化，却忽略了基于注意力机制的方法在问题参数异构分布下的鲁棒性局限。它们相对于启发式搜索的改进在很大程度上仍局限于人工策划的、固定分布的基准测试。此外，当节点表示高度相似或任务涉及长决策周期时，这些架构的性能往往会显著下降。为解决上述局限性，我们提出了一种新型融合神经网络框架，该框架采用离散噪声图扩散模型来学习车辆路径问题的潜在约束，并生成约束分配矩阵。该矩阵随后被自适应地集成到自回归求解器的特征表示学习和决策过程中，作为图结构掩码，促进形成具有全局视野和局部特征融合特征的解。据我们所知，这项工作代表了首次在 CVRPlib 公开数据集内跨越四个不同维度的 378 个组合空间对神经网络模型求解器进行的全面实验研究。广泛的实验评估表明，我们提出的融合模型有效地捕捉并利用了问题约束，在多个基准数据集上实现了最先进的性能。
