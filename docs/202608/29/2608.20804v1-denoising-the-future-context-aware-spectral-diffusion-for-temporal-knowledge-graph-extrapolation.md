---
title: "Denoising the Future: Context-Aware Spectral Diffusion for Temporal Knowledge Graph Extrapolation"
title_zh: 去噪未来：用于时序知识图谱外推的上下文感知频谱扩散
authors: "Yanglei Gan, Peng He, Run Lin, Peiyuan Jiang, Yifan Wang, Qiao Liu"
date: 2026-08-21
pdf: "https://arxiv.org/pdf/2608.20804v1"
tags: ["query:ardm"]
score: 6.0
evidence: 基于扩散模型的历史关系未来对象预测
tldr: 针对时序知识图谱（TKG）外推中历史信息噪声干扰预测信号的问题，本文提出FreqDiff框架。该框架将未来实体预测建模为查询槽去噪过程，并设计了双流去噪器，结合时间依赖建模与上下文感知的频谱校准。通过频谱分支自适应过滤历史噪声并利用频域正则化对齐目标，FreqDiff在四个主流基准数据集上达到了最优性能，有效提升了时序推理的准确性。
selection_source: fresh_fetch
motivation: 现有的扩散模型在处理TKG外推时，难以从复杂的历史背景中区分出与特定查询相关的关键证据，导致预测信号被非显著性事实稀释。
method: 提出FreqDiff框架，利用双流去噪器结合时间依赖与频谱校准，通过可学习的频域滤波器和正则化项来优化去噪表示并对齐目标实体。
result: 在四个公开的TKG基准数据集上的实验结果表明，FreqDiff显著优于现有方法，达到了当前最先进的性能水平。
conclusion: 结合频域分析与扩散模型能有效增强TKG外推中的特征提取与去噪能力，为处理时序数据的噪声干扰提供了创新的解决方案。
---

## 摘要
时序知识图谱（TKG）外推旨在从随时间变化的关系历史中推断未来的事实。最近基于扩散的方法通过生成式去噪改进了不确定性建模，但它们对主体历史的聚合条件建模可能无法充分区分查询特定的证据与非显著的历史事实，从而稀释了目标判别信号。为了弥补这一差距，我们提出了 FreqDiff，一种用于 TKG 外推的频率感知扩散框架。具体而言，FreqDiff 将未来实体预测建模为查询槽去噪，并开发了一个双流去噪器，该去噪器集成了时序依赖建模与上下文感知频谱校准。频谱分支从可学习基向量中合成历史条件滤波器，以自适应地重新校准去噪表示，同时提出了一种频域正则化器，用于在频谱空间中将去噪目标与真实实体对齐。在四个公开 TKG 基准数据集上的实验表明，FreqDiff 达到了最先进的性能。

## 速览
**TLDR**：针对时序知识图谱（TKG）外推中历史信息噪声干扰导致目标信号稀释的问题，本文提出 FreqDiff 框架。该框架将未来预测建模为查询槽去噪过程，并设计了双流去噪器，结合时间依赖建模与上下文感知频谱校准。通过学习频谱滤波器和频域正则化，FreqDiff 能有效从复杂历史中提取关键证据，在四个公开数据集上达到了 SOTA 性能。 \
**Motivation**：现有的基于扩散的 TKG 方法在聚合历史信息时，难以区分查询相关的证据与非显著事实，导致目标信号被稀释。 \
**Method**：提出 FreqDiff 框架，利用双流去噪器结合时间依赖建模与频谱校准，并通过频域正则化在频谱空间对齐预测目标。 \
**Result**：在四个主流 TKG 基准数据集上的实验结果表明，FreqDiff 显著优于现有方法，取得了最先进的预测性能。 \
**Conclusion**：结合频域分析与扩散模型能更有效地捕捉时序知识图谱中的关键演化模式，提升了未来事实预测的准确性与鲁棒性。

---

## Abstract
Temporal Knowledge Graph (TKG) extrapolation seeks to infer future facts from time-varying relational histories. Recent diffusion-based approaches improve uncertainty modeling through generative denoising, but their aggregated conditioning on subject histories may insufficiently distinguish query-specific evidence from non-salient historical facts, thereby diluting target-discriminative signals. To bridge this gap, we propose FreqDiff, a Frequency-aware Diffusion framework for TKG extrapolation. Specifically, FreqDiff formulates future object prediction as query-slot denoising and develops a dual-stream denoiser that integrates temporal dependency modeling with context-aware spectral calibration. The spectral branch synthesizes history-conditioned filters from learnable bases to adaptively re-calibrate denoising representations, while a frequency-domain regularizer is proposed to align the denoised target with the gold object in spectral space. Experiments on four public TKG benchmarks demonstrate that FreqDiff achieves state-of-the-art performance.