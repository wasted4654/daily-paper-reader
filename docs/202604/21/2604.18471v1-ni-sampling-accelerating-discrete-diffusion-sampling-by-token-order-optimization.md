...
tldr: 离散扩散语言模型（dLLM）虽具备并行解码潜力，但现有启发式采样策略效率较低。本文提出神经指示器采样（NI Sampling）框架，通过优化标记采样顺序来加速生成。该方法利用神经指示器动态预测每步应确定的标记，并结合轨迹保持目标进行训练。实验表明，NI Sampling在保持高精度的同时，实现了高达14.3倍的采样加速，显著优于传统的置信度采样方法。
selection_source: fresh_fetch
motivation: 针对离散扩散模型中现有采样策略因未能充分利用预测准确性而导致生成效率低下的问题。
method: 提出一种通用的采样顺序优化框架，通过神经指示器决定每步采样的标记，并引入轨迹保持目标函数进行优化。
result: 在多个基准测试中，该方法相比全步采样实现了最高14.3倍的加速，且在精度与步数的平衡上表现优异。
conclusion: 优化标记采样顺序是提升离散扩散模型效率的关键，NI Sampling为实现高效、高质量的并行文本生成提供了有效方案。
---

## 摘要
离散扩散语言模型 (dLLMs) 最近已成为传统自回归方法的一种极具前景的替代方案，它提供了以任意顺序生成 token 的灵活性以及并行解码的潜力。然而，现有的启发式采样策略仍然效率低下：它们在每一步仅选择一小部分 token 进行采样，留下了巨大的改进空间。在这项工作中，我们研究了 token 采样顺序优化问题，并展示了其在加速方面的巨大潜力。具体而言，我们发现充分利用每一步的正确预测，可以在不损失准确性的情况下，将采样迭代次数减少一个数量级。基于此，我们提出了神经指示器采样 (Neural Indicator Sampling, NI Sampling)，这是一个通用的采样顺序优化框架，利用神经指示器来决定每一步应该采样哪些 token。我们进一步提出了一种新颖的轨迹保持目标来训练该指示器。在 LLaDA 和 Dream 模型上的多个基准测试实验表明，我们的方法在性能下降可以忽略不计的情况下，比全步采样实现了高达 14.3 倍的加速，并且在准确率与步数的权衡中始终优于置信度阈值采样。代码可在 https://github.com/imagination-research/NI-Sampling 获取。
